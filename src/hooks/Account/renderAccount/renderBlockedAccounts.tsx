...
async function unBlock(account: string, currentAccount: SubscribedPosts){
    try{
        ...
        await api.updateDocument(import.meta.env.VITE_REACT_APP_SUBSCRIBE_DATABASE_ID, import.meta.env.VITE_REACT_APP_SUBSCRIBE_COLLECTION_ID, currentAccount.$id, data);
...
export default function RenderBlockedAccounts(){
    ...
    useEffect(()=>{
            const findAccount = subscribedPosts.find((subscribedPosts: SubscribedPosts)=>user.email || subscribedPosts.id === getEmail());
            setCurrentAccount(findAccount);
    },[subscribedPosts])

    const listOfBlockedAccounts = currentAccount?.blocked.map((account: string)=>{
        return(
            <section key = {account}>
                <h3>{account}</h3>
                {Button({text: "Unblock", onClick: ()=>unBlock(account, currentAccount), classNames: "button"})}
            </section>
        )
    });
